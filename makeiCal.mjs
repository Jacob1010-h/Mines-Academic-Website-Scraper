...
const main = async () => {
  try {
    // Await the result of getCalendar
    const academicCalendar = await getCalendar(options);
    console.log("Fetched academic calendar:", academicCalendar);

    // Generate the iCal content
    const icsFileContent = makeCalendar(academicCalendar);
